...
impl Clone for TypeInfo {
    fn clone(&self) -> TypeInfo {
        unsafe {
            TypeInfo {
                ptr: ddsi_typeinfo_dup(self.ptr),
            }
        }
    }
}
    ...
    pub(crate) fn create_forwarding_writer(
        ...
        unsafe {
            ...
            if self.local_filter_mode == LocalFilterMode::IgnoreLocalQos {
                println!("Setting ignore local filter for {}:{}", topic_name, type_name);
...
pub(crate) fn serialize_entity_guid<S>(entity: &dds_entity_t, s: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
